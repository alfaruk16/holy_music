...
class GetMyFavoritesUseCase @Inject constructor(private val apiRepo: ApiRepo) {

    operator fun invoke(): kotlinx.coroutines.flow.Flow<Async<TracksDto>> = flow {
        try {
            emit(Async.Loading())
            val tracks = apiRepo.getMyFavorites()
