...
class GetScholarTrackByArtistUseCase @Inject constructor(private val apiRepo: ApiRepo) {

    operator fun invoke(artistId: String, type: String): Flow<Async<TracksDto>> = flow {
        try {
            emit(Async.Loading())
            val tracks = apiRepo.getScholarTracksByArtist(artistId, type)
            emit(Async.Success(tracks.copy(data = tracks.data?.filter { it.isActive ?: false }?.reversed())))
