...
class GetSingleKhatamUseCase @Inject constructor(private val apiRepo: ApiRepo) {
    operator fun invoke(id: String): Flow<Async<SingleKhatamDto>> = flow {
        try {
            emit(Async.Loading())
            val khatamDto = apiRepo.getSingleKhatam(id = id)
            emit(Async.Success(khatamDto))
