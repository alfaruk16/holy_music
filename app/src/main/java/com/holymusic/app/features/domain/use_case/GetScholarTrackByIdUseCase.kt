...
class GetScholarTrackByIdUseCase @Inject constructor(private val apiRepo: ApiRepo) {

    operator fun invoke(id: String): Flow<Async<ScholarTrackDto>> = flow {
        try {
            emit(Async.Loading())
            val artist = apiRepo.getScholarTrackById(id = id)
