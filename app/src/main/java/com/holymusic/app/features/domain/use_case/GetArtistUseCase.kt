...
class GetArtistUseCase @Inject constructor(private val apiRepo: ApiRepo) {
    operator fun invoke(): Flow<Async<ArtistDto>> = flow {
        try {
            emit(Async.Loading())
            val artist = apiRepo.getArtist()
            emit(Async.Success(artist.copy(data = artist.data?.filter { it.isActive == true }
