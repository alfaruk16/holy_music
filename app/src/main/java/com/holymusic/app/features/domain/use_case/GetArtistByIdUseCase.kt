...
class GetArtistByIdUseCase @Inject constructor(private val apiRepo: ApiRepo) {

    operator fun invoke(id: String): Flow<Async<SingleArtistDto>> = flow {
        try {
            emit(Async.Loading())
            val artist = apiRepo.getArtistById(id = id)
