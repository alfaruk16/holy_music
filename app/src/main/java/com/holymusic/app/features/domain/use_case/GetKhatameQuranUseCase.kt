...
class GetKhatameQuranUseCase @Inject constructor(private val apiRepo: ApiRepo) {

    operator fun invoke(): Flow<Async<KhatameQuranDto>> = flow {
        try {
            emit(Async.Loading())
            val communityAll = apiRepo.getKhatameQuran()
            emit(Async.Success(communityAll.copy(data = communityAll.data?.filter { it.isActive ?: false }?.sortedBy { it.sequenceNo })))
