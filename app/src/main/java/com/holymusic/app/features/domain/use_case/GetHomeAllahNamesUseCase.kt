...
class GetHomeAllahNamesUseCase @Inject constructor(private val apiRepo: ApiRepo) {
    operator fun invoke(): Flow<Async<AllahNamesDto>> = flow {
        try {
            emit(Async.Loading())
            val names = apiRepo.getHomeAllahNames()
            emit(Async.Success(names))
