...
class RobiPinVerifyUseCase @Inject constructor(private val paymentApiRepo: PaymentApiRepo) {
    operator fun invoke(body: PinVerify): kotlinx.coroutines.flow.Flow<Async<TelcoResponseDto>> = flow {
        try {
            emit(Async.Loading())
            val ssl = paymentApiRepo.robiPinVerify(body)
