...
class GetScholarTrackByType @Inject constructor(private val apiRepo: ApiRepo) {

    operator fun invoke(type: String): Flow<Async<TracksDto>> = flow {

        try {
            emit(Async.Loading())
            val tracks = apiRepo.getScholarTracksByType(type = type)
