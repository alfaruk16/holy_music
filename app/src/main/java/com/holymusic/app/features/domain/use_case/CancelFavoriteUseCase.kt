...
class CancelFavoriteUseCase @Inject constructor(private val apiRepo: ApiRepo) {

    operator fun invoke(trackId: String): Flow<Async<DefaultDto>> = flow {
        try {
            emit(Async.Loading())
            val cancelFavorite = apiRepo.cancelFavorite(trackId)
            emit(Async.Success(cancelFavorite))
