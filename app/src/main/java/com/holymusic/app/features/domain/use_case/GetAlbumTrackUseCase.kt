...
class GetAlbumTrackUseCase @Inject constructor(private val apiRepo: ApiRepo) {

    operator fun invoke(id: String): Flow<Async<AlbumTrackDto>> = flow {
        try {
            emit(Async.Loading())
            val tracks = apiRepo.getAlbumTrack(id)
