...
class GetContentsUseCase @Inject constructor(private val apiRepo: ApiRepo){

    operator fun invoke(): Flow<Async<ContentsDto>> = flow {
        try {
            emit(Async.Loading())
            val contents = apiRepo.getContents()
            emit(Async.Success(contents))
