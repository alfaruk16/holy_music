...
class GetImageContentsUseCase @Inject constructor(private val apiRepo: ApiRepo) {

    operator fun invoke(id: String): Flow<Async<ImageContentsDto>> = flow {
        try {
            emit(Async.Loading())
            val prayerTimes = apiRepo.getImageContents(id)
            emit(Async.Success(prayerTimes.copy(data = prayerTimes.data?.filter { it.isActive ?: false }?.reversed())))
